...
public class NoozCameraFragment extends CameraFragment {
	private static final String KEY_USE_FFC = "com.nooz.nooz.mediarecorder.camerafragment.USE_FFC";
	...
	static NoozCameraFragment newInstance(boolean useFFC) {
		NoozCameraFragment f = new NoozCameraFragment();
	...
	class NoozCameraHost extends SimpleCameraHost implements Camera.FaceDetectionListener {
		...
		public Parameters adjustPreviewParameters(Parameters parameters) {
			flashMode = CameraUtils.findBestFlashModeMatch(parameters, Camera.Parameters.FLASH_MODE_RED_EYE,
		...
		public void onFaceDetection(Face[] faces, Camera camera) {
			if (faces.length > 0) {
				...
				if (now > lastFaceToast + 10000) {
					Toast.makeText(getActivity(), "I see your face!", Toast.LENGTH_LONG).show();
					lastFaceToast = now;
				}
			}
		}
